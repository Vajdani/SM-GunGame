...
Melee.swingCount = 2
Melee.mayaFrameDuration = 1.0 / 30.0
Melee.freezeDuration = 0.075

Melee.swings = { "sledgehammer_attack1", "sledgehammer_attack2" }
Melee.swingFrames = { 4.2 * Melee.mayaFrameDuration, 4.2 * Melee.mayaFrameDuration }
Melee.swingExits = { "sledgehammer_exit1", "sledgehammer_exit2" }

function Melee.client_onCreate(self)
	self.isLocal = self.tool:isLocal()
	self:init()
end

function Melee.client_onRefresh(self)
	self:init()
	self:loadAnimations()
end

function Melee.init(self)
...
function Melee.loadAnimations(self)
...
function Melee.client_onUpdate(self, dt)
	...
	if self.isLocal then
		...
		if preAnimation ~= self.fpAnimations.currentAnimation then
			...
			elseif isAnyOf(self.fpAnimations.currentAnimation, { "guardInto", "guardIdle", "guardExit", "guardBreak",
					"guardHit" }) then
				keepBlockSprint = true
			end
		...
		if isSprinting and self.fpAnimations.currentAnimation == "idle" and self.attackCooldownTimer <= 0 and not isAnyOf(self.fpAnimations.currentAnimation, {
				"sprintInto", "sprintIdle" }) then
...
function Melee.updateFreezeFrame(self, state, dt)
...
function Melee.server_startEvent(self, params)
...
function Melee.client_startLocalEvent(self, params)
	self:client_handleEvent(params)
end

function Melee.client_handleEvent(self, params)
...
--function Melee.sv_n_toggleTumble( self )
--	local character = self.tool:getOwner().character
--	character:setTumbling( not character:isTumbling() )
--end

function Melee.client_onEquippedUpdate(self, primaryState, secondaryState)
	...
	if self.pendingRaycastFlag then
		...
		if time >= frameTime and frameTime ~= 0 then
			...
			sm.melee.meleeAttack(melee_sledgehammer, self.damage, raycastStart, direction * self.range,
			self.tool:getOwner())
...
function Melee.client_onEquip(self, animate)
	if animate then
	...
	for k, v in pairs(self.renderables) do renderablesTp[#renderablesTp + 1] = v end
	for k, v in pairs(self.renderables) do renderablesFp[#renderablesFp + 1] = v end
...
function Melee.client_onUnequip(self, animate)
	self.equipped = false
...
Sledgehammer = class(Melee)
Sledgehammer.swingSpeed = 1
Sledgehammer.range = 3
Sledgehammer.damage = 20
Sledgehammer.renderables = { "$GAME_DATA/Character/Char_Tools/Char_sledgehammer/char_sledgehammer.rend" }
